...
//import com.tm.videostream.enums.Condition;
...
public interface VideoRepository extends JpaRepository<Video, Integer> {
	...
	@Query("SELECT new com.tm.videostream.dto.VideoDTO(video.fileId, video.title, video.description, "
			+ "video.filename, user.username) FROM Video video WHERE video.user.username = :username "
	...
	@Query("SELECT new com.tm.videostream.dto.VideoDTO(video.fileId, video.title, video.description, "
	        + "video.filename, user.username) FROM Video video "
	        + "INNER JOIN User user ON video.user.userId = user.userId "
	        + "WHERE  video.size < :size "
	        + "AND video.approvalStatus = :approvalStatus "
	        + "ORDER BY video.updatedAt DESC")
	List<VideoDTO> findAdminVideoBySize(float size, String approvalStatus);
